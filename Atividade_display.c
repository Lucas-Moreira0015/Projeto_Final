...
const uint botao_A = 5; // Botão de abrir/fechar a porta
const uint botao_B = 6; // Botão para verificar proximidade do dono
const uint joystick_x = 26; // Pino do eixo X do joystick (ADC)
const uint joystick_y = 27; // Pino do eixo Y do joystick (ADC)
const uint botao_joystick = 22; // Pino do botão do joystick
...
void atualizar_display() {
    ...
    if (movimento_detectado) {
        ssd1306_draw_string(&ssd, "Movimento Detectado", 10, 30);
    }
    if (alarme_ativado) {
...
void gpio_irq_handler(uint gpio, uint32_t events) {
    ...
    if (current_time - last_time > 300000) {
        ...
        // Verifica se o botão A foi pressionado (abre ou fecha a porta)
        if (gpio == botao_A && !gpio_get(botao_A)) {
            if (dono_proximo) { // Só abre/fecha se o dono estiver próximo
        ...
        // Verifica se o botão B foi pressionado (verifica proximidade do dono)
        if (gpio == botao_B && !gpio_get(botao_B)) {
            ...
            // Se o alarme estiver ativado e o dono estiver próximo, desativa o alarme
...
// Função para ler o valor do eixo X ou Y do joystick
...
void processar_comando_serial(char comando) {
    switch (comando) {
        case 'A': // Abrir a porta
            if (dono_proximo) {
                porta_aberta = true;
                led_verde_on = true;
                gpio_put(led_verde, led_verde_on);
            ...
            break;
        case 'F': // Fechar a porta
            porta_aberta = false;
            led_verde_on = false;
            gpio_put(led_verde, led_verde_on);
            atualizar_display();
            break;
        case 'L': // Ligar o alarme
            alarme_ativado = true;
...
int main() {
    ...
    gpio_pull_up(botao_A); // Aciona o resistor de pull-up para o botão A
    gpio_init(botao_B);
    gpio_set_dir(botao_B, GPIO_IN);
    gpio_pull_up(botao_B); // Aciona o resistor de pull-up para o botão B
    gpio_init(botao_joystick);
    gpio_set_dir(botao_joystick, GPIO_IN);
    gpio_pull_up(botao_joystick); // Aciona o resistor de pull-up para o botão do joystick
    gpio_init(buzzer);
    ...
    while (true) {
        ...
        if (valor_x < 1900 || valor_x > 2050 || valor_y < 1950 || valor_y > 2250) {
            ...
            gpio_put(led_verde, led_verde_on);
            atualizar_display();
            sleep_ms(5000); // Mantém o LED azul ligado por 5 segundos
        ...
        if (uart_is_readable(UART_ID)) {
            ...
            processar_comando_serial(comando);
        }
